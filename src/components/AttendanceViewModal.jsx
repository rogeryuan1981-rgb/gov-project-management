...
export default function AttendanceViewModal({ isOpen, onClose, selectedProject, personnel = [], allExistingUnits = [] }) {
  ...
  const filteredRecords = records.filter(r => {
    ...
    return matchesName && (selectedUnit === 'ALL' || r.unit === selectedUnit);
  });
    ...
    <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center p-4 animate-in fade-in">
      <div className="bg-white dark:bg-slate-800 w-full max-w-6xl rounded-3xl shadow-2xl border border-slate-200 dark:border-slate-700 overflow-hidden flex flex-col h-[85vh]">
        ...
        <div className="px-6 py-4 border-b border-slate-100 dark:border-slate-700 flex justify-between items-center bg-slate-50 dark:bg-slate-800/80">
          <div className="flex items-center space-x-2">
            ...
            <div>
              ...
              <p className="text-xs text-slate-400 mt-0.5">系統已完成人名空白字元除錯（Trim Line）。依據系統名冊之主金鑰整合打卡數據。</p>
            </div>
          </div>
        ...
        <div className="overflow-y-auto flex-1 p-6 bg-slate-50/30 dark:bg-slate-900/10">
          {isLoading ? (
            <div className="flex flex-col items-center justify-center h-full space-y-2"><Loader2 size={36} className="text-indigo-500 animate-spin" /><span className="text-xs text-slate-400">正在整合全量人名聯集整合中...</span></div>
          ) : sortedRecords.length === 0 ? (
            <div className="flex flex-col items-center justify-center h-full text-center py-12"><FileText size={48} className="text-slate-300 dark:text-slate-600 mb-3" /><p className="text-sm font-bold text-slate-700 dark:text-slate-300">目前查無符合條件之名冊或刷卡紀錄</p></div>
          ) : (
                  ...
                  {sortedRecords.map((r) => {
                    ...
                    return (
                      <tr key={r.id} className={rowBg}>
                        ...
                        <td className="py-3.5 px-4">{isRowEditing ? <input type="text" placeholder="ex. 08:30" value={editFormData.checkIn} onChange={e => setEditFormData({...editFormData, checkIn: e.target.value})} className="px-2 py-1 w-20 bg-white dark:bg-slate-900 border border-slate-200 rounded text-xs font-mono border-indigo-300 text-slate-800 dark:text-white" /> : <span className="font-mono">{r.checkIn || '--'}</span>}</td>
                        <td className="py-3.5 px-4">{isRowEditing ? <input type="text" placeholder="ex. 17:30" value={editFormData.checkOut} onChange={e => setEditFormData({...editFormData, checkOut: e.target.value})} className="px-2 py-1 w-20 bg-white dark:bg-slate-900 border border-slate-200 rounded text-xs font-mono border-indigo-300 text-slate-800 dark:text-white" /> : <span className="font-mono">{r.checkOut || '--'}</span>}</td>
                        <td className="py-3.5 px-4">{isRowEditing ? <input type="text" placeholder="ex. 16:38~17:38" value={editFormData.leaveRangeInfo} onChange={e => setEditFormData({...editFormData, leaveRangeInfo: e.target.value})} className="px-2 py-1 w-36 bg-white dark:bg-slate-900 border border-slate-200 rounded text-xs border-indigo-300 text-slate-800 dark:text-white" /> : <span className="text-slate-500 dark:text-slate-400 truncate block max-w-[150px]">{r.leaveRangeInfo || '--'}</span>}</td>
                        <td className="py-3.5 px-4">
                          {isRowEditing ? (
                            <select value={editFormData.leaveType} onChange={e => setEditFormData({...editFormData, leaveType: e.target.value})} className="px-2 py-1 bg-white dark:bg-slate-700 border border-slate-200 rounded text-xs text-slate-800 dark:text-slate-100 font-bold focus:border-indigo-500">
